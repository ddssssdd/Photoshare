var UserProfileView=function(user,tabName){
	...
	var followersLabel = Ti.UI.createLabel({
		...
		text:(user.followed_count>1? LL('email.daily.follower.title') : LL('userhome.follower') )
	});
	...
	var loadBoards=function(datas){
		...
		if (toolBar){
			toolBar.labels[0]= datas.length+(datas.length>1? LL('user.boards') : LL('app.collection') );
		}
		...
		for(var i=0;i<datas.length;i++){
			...
			row.board = item;
			row.add(label);
			var info =item.pins+(item.pins>1? LL('user.pins') : LL('user.pin') )+" "+item.followers+(item.followers>1? LL('email.daily.follower.title') : LL('userhome.follower') );
	...
	var loadLikes=function(datas){
		if (toolBar){
			toolBar.labels[2]= datas.length+(datas.length>1? LL('user.likes') : LL('app.like') );
		}
	...
	if (Titanium.Platform.name == 'iPhone OS')
	{
		...
		var CreateToolBar=function(likecount,pincount,collectioncount){
			toolBar= Titanium.UI.iOS.createTabbedBar({
				...
				width : 300,
				height : 25,
				labels : [collectioncount+ (collectioncount>1? LL('user.boards') : LL('app.collection') ),
						 pincount+(pincount>1? LL('user.pins') : LL('user.pin') ), 
						 likecount+(likecount>1? LL('user.likes') : LL('app.like'))],
